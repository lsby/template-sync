...
export let 管理员交互式人工核验用户管理界面流程 = new 流程<演示需求流程上下文, 演示需求依赖>({
  ...
  步骤们: [
    new 观察('人工核对用户管理界面排版与功能可用性', async ({ 系统 }) => {
      let 审核结果 = await 演示_确认(
        系统.page,
        '【人工交互核验】请检查当前用户管理界面：\n1. 表格各列（ID、名称）数据对齐正常且无排版错位\n2. 顶部“添加数据”与行内“编辑/删除/修改密码”操作均清晰可见\n点击【是】确认验收通过，点击【否】判定验收未通过。',
      )
      let 证据内容 = {
        判定: 审核结果.通过 ? '通过' : '未通过',
        ...(审核结果.附加说明 === undefined ? {} : { 附加说明: 审核结果.附加说明 }),
      }
      if (审核结果.通过 === false) {
        return {
          通过: false,
          原因: `人工交互核验未通过：审核员在确认框中选择了【否】${审核结果.附加说明 === undefined ? '' : `；${审核结果.附加说明}`}`,
          证据们: [{ 手段: '人工检查', 描述: '审核员拒绝了用户管理界面验收', 内容: 证据内容 }],
        }
      }
      return {
        通过: true,
        证据们: [{ 手段: '人工检查', 描述: '审核员在线通过了用户管理界面布局与交互核验', 内容: 证据内容 }],
      }
    }),
  ],

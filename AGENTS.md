...
- 单元测试:
  ...
  - 通过命令 `test:unit`，脚本将自动搜集所有Co-location测试文件，生成测试聚合文件
- 集成测试:
...
- 需求测试:
  ...
  - 修改需求模型后运行 `npm run task -- test:requirement:coverage`，演示运行使用 `npm run test:requirement`，无头运行全部流程使用 `npm run test:requirement:all`。

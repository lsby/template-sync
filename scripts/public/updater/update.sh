...
APP_DIR="$ROOT/template-sync.app"
DATA_DIR="$ROOT/data"
DB_DIR="$DATA_DIR/db"
...
APP_PROCESS_NAME="template-sync"
...
expand_validated_zip() {
  ...
  local staged_app="$STAGING_DIR/template-sync.app"
  local staged_pkg="$staged_app/Contents/Resources/app/package.json"
  local staged_bin="$staged_app/Contents/MacOS/template-sync"
...
invoke_prisma() {
  ...
  local electron_bin="$macos_dir/template-sync-bin"
...
STAGED_APP_DIR="$STAGING_DIR/template-sync.app"
...
if [ -d "$ROOT/template-sync.app" ]; then
  chmod -R +x "$ROOT/template-sync.app/Contents/MacOS" 2>/dev/null || true
  xattr -dr com.apple.quarantine "$ROOT/template-sync.app" 2>/dev/null || true
fi

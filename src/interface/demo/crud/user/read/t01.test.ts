...
export default new 接口逻辑测试(
  ...
  async (): Promise<void> => {
    let 结果 = await 接口
      .获得接口逻辑()
      .调用({ userId: adminId, kysely: kysely管理器, json: { page: 1, size: 10 } }, {}, 默认请求附加参数)
    assert.strictEqual(结果.isRight(), true)
    let 数据 = 结果.assertRight().getRight()
    assert.strictEqual(数据.data.length, 1)
    assert.strictEqual(数据.total, 1)
  },
)

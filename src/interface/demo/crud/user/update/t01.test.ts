...
export default new 接口逻辑测试(
  ...
  async (): Promise<void> => {
    let 结果 = await 接口
      .获得接口逻辑()
      .调用({ userId: id, kysely: kysely管理器, json: { userId: id, newName: 'newAdmin' } }, {}, 默认请求附加参数)
    assert.strictEqual(结果.isRight(), true)
    let db = kysely管理器.获得句柄()
    let row = await db.selectFrom('user').select('name').where('id', '=', id).executeTakeFirst()
    assert.strictEqual(row?.name, 'newAdmin')
  },
)
